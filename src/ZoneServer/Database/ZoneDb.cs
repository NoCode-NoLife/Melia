...
namespace Melia.Zone.Database
{
	public class ZoneDb : MeliaDb
	{
		...
		/// <summary>
		/// Returns an characterId for a given ItemUniqueId.
		/// </summary>
		/// <param name="itemUniqueId"></param>
		/// <returns></returns>
		public long GetCharacterIdByItemUniqueId(long itemUniqueId)
		{
			using (var conn = this.GetConnection())
			{
				using (var cmd = new MySqlCommand("SELECT * FROM `inventory` WHERE `itemId` = @itemUniqueId", conn))
				{
					...
					using (var reader = cmd.ExecuteReader())
					{
						while (reader.Read())
						{
							var characterId = reader.GetInt64("characterId");
		...
		/// <param name="itemUniqueId"></param>
		/// <returns></returns>
		public void RemoveItem(long itemUniqueId, int removalAmount)
		{
			int newAmount = 0;

			using (var conn = this.GetConnection())
			{
				using (var cmd = new MySqlCommand("SELECT * FROM `items` WHERE `itemUniqueId` = @itemUniqueId", conn))
				{
					cmd.AddParameter("@itemUniqueId", itemUniqueId);
					...
					using (var reader = cmd.ExecuteReader())
					{
						while (reader.Read())
						{
							var itemAmount = reader.GetInt32("amount");
							if (itemAmount < removalAmount)
							{
								removalAmount = itemAmount;
							}

							newAmount = itemAmount - removalAmount;
						}
					}
				}
			}
			...
			if (newAmount <= 0)
			{
				using (var conn = this.GetConnection())
				{
					using (var trans = conn.BeginTransaction())
					{
						using (var cmdItems = new MySqlCommand("DELETE FROM `items` WHERE `itemUniqueId` = @itemUniqueId", conn, trans))
						{
							cmdItems.AddParameter("@itemUniqueId", itemUniqueId);
							cmdItems.ExecuteNonQuery();
						}

						using (var cmdInventory = new MySqlCommand("DELETE FROM `inventory` WHERE `itemId` = @itemId", conn, trans))
						{
							cmdInventory.AddParameter("@itemId", itemUniqueId);
			...
			else
			{
				using (var conn = this.GetConnection())
				{
					using (var cmdUpdate = new UpdateCommand("UPDATE `items` SET {0} WHERE `itemUniqueId` = @itemUniqueId", conn))
					{
						cmdUpdate.AddParameter("@itemUniqueId", itemUniqueId);
						cmdUpdate.Set("amount", newAmount);
