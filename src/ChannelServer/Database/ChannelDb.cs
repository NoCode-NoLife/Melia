	...
	public class ChannelDb : MeliaDb
	{
		...
		public Character GetCharacter(long accountId, long characterId)
		{
			...
			using (var mc = new MySqlCommand("SELECT * FROM `characters` WHERE `accountId` = @accountId AND `characterId` = @characterId", conn))
			{
				...
				using (var reader = mc.ExecuteReader())
				{
					...
					character.BaseStat.MaxHp = reader.GetInt32("maxHp");
					character.BaseStat.Hp = reader.GetInt32("hp");
					character.BaseStat.MaxSp = reader.GetInt32("maxSp");
					character.BaseStat.Sp = reader.GetInt32("sp");
					character.BaseStat.MaxStamina = reader.GetInt32("maxStamina");
					character.BaseStat.Stamina = reader.GetInt32("stamina");
		...
		public bool SaveCharacter(Character character)
		{
			...
			using (var cmd = new UpdateCommand("UPDATE `characters` SET {0} WHERE `characterId` = @characterId", conn))
			{
				...
				cmd.Set("maxHp", character.BaseStat.MaxHp);
				cmd.Set("sp", character.BaseStat.Sp);
				cmd.Set("maxSp", character.BaseStat.MaxSp);
				cmd.Set("stamina", character.BaseStat.Stamina);
				cmd.Set("maxStamina", character.BaseStat.MaxStamina);
