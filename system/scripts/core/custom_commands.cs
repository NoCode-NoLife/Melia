...
public class CustomCommandFunctionsScript : GeneralScript
{
	...
	public CustomCommandResult SCR_PUT_CAMPFIRE(Character character, int numArg1, int numArg2, int numArg3)
	{
		...
		var monsters = character.Map.GetAttackableEntitiesIn(character, area);

		foreach (var monster in monsters)
		{
			var mob = monster as Mob;
			// Another camp fire is near by
			if (mob.Id == 46011)
			{
	...
	private async void BonFireSpawn(Character character)
	{
		...
		Send.ZC_ENTER_MONSTER(bonFire);
		Send.ZC_FACTION(character.Connection, bonFire, FactionType.Neutral);

		var cancellationTokenSource = new CancellationTokenSource();

		this.AddBuffToNearByPlayers(character, cancellationTokenSource.Token);

		await Task.Delay(TimeSpan.FromMinutes(5));

		cancellationTokenSource.Cancel();
		Send.ZC_DEAD(bonFire);
		character.Map.RemoveMonster(bonFire);
	}

	private async void AddBuffToNearByPlayers(Character character, CancellationToken cancellationToken)
	{
		while (!cancellationToken.IsCancellationRequested)
		{
			var area = new Circle(character.Position, 350);
			var characters = character.Map.GeCharactersIn(character, area);
