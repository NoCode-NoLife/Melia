function COMPANIONSHOP_ADOPT_PAGE_RIGHT(frame, ctrl, argstr, argnum)
	local topFrame = frame:GetTopParentFrame()
	local currentPage = tonumber(topFrame:GetUserValue('COMPANION_SHOP_PAGE')) or 1
	local totalPages = tonumber(topFrame:GetUserValue('COMPANION_SHOP_TOTAL_PAGES')) or 1
	if argnum == 1 then
		topFrame:SetUserValue('COMPANION_SHOP_PAGE', totalPages)
