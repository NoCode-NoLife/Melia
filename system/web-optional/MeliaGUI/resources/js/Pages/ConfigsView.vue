...
const slideNext = () => {
  state.currentIndex = Math.min(state.currentIndex + 1, state.pageTitles.length - 1);
};
...
const saveConfigs = () => {
    ...
    const worldConfigs = Object.keys(instance.props.configs.world).reduce((acc, key) => {
        const { value } = instance.props.configs.world[key];
        acc[key] = value;
        return acc;
    }, {});

    const barracksConfigs = Object.keys(instance.props.configs.barracks).reduce((acc, key) => {
        const { value } = instance.props.configs.barracks[key];
        acc[key] = value;
        return acc;
    }, {});

    const webConfigs = Object.keys(instance.props.configs.web).reduce((acc, key) => {
        const { value } = instance.props.configs.web[key];
        acc[key] = value;
        return acc;
    }, {});

    const databaseConfigs = Object.keys(instance.props.configs.database).reduce((acc, key) => {
        const { value } = instance.props.configs.database[key];
        acc[key] = value;
        return acc;
    }, {});

    const commandsConfigs = Object.keys(instance.props.configs.commands).reduce((acc, key) => {
        const { value } = instance.props.configs.commands[key];
        acc[key] = value;
        return acc;
    }, {});

    const logConfigs = Object.keys(instance.props.configs.log).reduce((acc, key) => {
        const { value } = instance.props.configs.log[key];
        acc[key] = value;
        return acc;
    }, {});

    const localizationConfigs = Object.keys(instance.props.configs.localization).reduce((acc, key) => {
        const { value } = instance.props.configs.localization[key];
        acc[key] = value;
        return acc;
    }, {});

    const mergedConfigs = { barracks: barracksConfigs, world: worldConfigs, web: webConfigs, database: databaseConfigs, commands: commandsConfigs, log: logConfigs, localization: localizationConfigs };
            ...
            <div v-if="isServerOnline" class="slider">
                <div class="slide">
                    <div lass="grid grid-cols-1 gap-6">
                        <SectionTitleLineWithButton :icon="mdiCog" :title="state.pageTitles[state.currentIndex]" main>
                        ...
                        <CardBox>
                            ...
                            <div v-if="state.currentIndex === 1" v-for="(entry, key) in instance.props.configs.web" :key="key">
                                <FormField :label="key" :help="entry.commentary">
                                    <FormControl
                            ...
                            <div v-if="state.currentIndex === 2" v-for="(entry, key) in instance.props.configs.barracks" :key="key">
                                <FormField :label="key" :help="entry.commentary">
                                    <FormControl
                            ...
                            <div v-if="state.currentIndex === 3" v-for="(entry, key) in instance.props.configs.database" :key="key">
                                <FormField :label="key" :help="entry.commentary">
                                    <FormControl
                            ...
                            <div v-if="state.currentIndex === 4" v-for="(entry, key) in instance.props.configs.commands" :key="key">
                                <FormField :label="key" :help="entry.commentary">
                                    <FormControl
                            ...
                            <div v-if="state.currentIndex === 5" v-for="(entry, key) in instance.props.configs.log" :key="key">
                                <FormField :label="key" :help="entry.commentary">
                                    <FormControl
                            ...
                            <div v-if="state.currentIndex === 6" v-for="(entry, key) in instance.props.configs.localization" :key="key">
                                <FormField :label="key" :help="entry.commentary">
                                    <FormControl
